...
En primera instancia, se dejaron las condiciones iniciales fijas ($x$(0) = -0.01 y $\dot{x}$(0) = 0.05). Como en el oscilador amortiguado, se utilizo una variable \textit{j} dentro del bucle \textit{for} para ir variando el parametro $\mu$. (mu1 = , mu2 = mu3 =). Cabe aclarar que \textit{j} tiene el mismo comportamiento que en el codigo para el oscilador amortiguado.

GRAFICOS PARA MU CHICOS VARIABLES y CI FIJAS


En una segunda instancia, las condiciones iniciales tambien se variaron dentro del mismo bucle. La siguiente tabla muestra los valores correspondientes a este caso:

TABLA CON VALOR PARA MU CHICOS VARIABLES y CI VARIABLES


\item $\mu$ > > 1:


Como en el caso anterior, primero se dejaron las condiciones iniciales fijas ($x$(0) = -0.01 y $\dot{x}$(0) = 0.05), variando el parametro $\mu$. (mu1 = , mu2 =, mu3 =)

TABLA CON VALOR PARA MU GRANDES VARIABLES y CI FIJAS


Luego, las condiciones iniciales tambien se variaron dentro del mismo bucle. La siguiente tabla muestra los valores correspondientes a este caso:

TABLA CON VALOR PARA MU GRANDES VARIABLES y CI VARIABLES
